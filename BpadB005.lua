...
local function createUI()
    if player.PlayerGui:FindFirstChild("BpadB005_AnimUI") then
        player.PlayerGui.BpadB005_AnimUI:Destroy()
    end

    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "BpadB005_AnimUI"
...
player.CharacterAdded:Connect(function(char)
    wait(0.5)
    if not player.PlayerGui:FindFirstChild("BpadB005_AnimUI") then
        createUI()
    end
end)

print("✅ BpadB005 Anim + BETON siap, UI persistent")
